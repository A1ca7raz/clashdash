...
test('login, create a Profile, and use a public subscription token', async ({ page, request }) => {
  ...
  await expect(importDialog).toContainText('lines[1]: Invalid proxy URI')
...
test('change the persisted password and optionally enable TOTP', async ({ page }) => {
  ...
  await expect(passwordCard.getByText('密码已更新。')).toBeVisible()
  ...
  await expect(restorePasswordCard.getByText('密码已更新。')).toBeVisible()
})
